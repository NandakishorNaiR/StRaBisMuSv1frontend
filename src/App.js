...
const CamIcon = () => ( <
    ...
    strokeWidth = "1.5" >
    <
    path d = "M23 7l-7 5 7 5V7z" / > < rect x = "1"
    y = "5"
    width = "15"
    height = "14"
    rx = "2" / >
...
function ResultCard({ result }) {
    ...
    return ( <
        ...
        <
        ConfBar label = "Normal"
        value = { probNormal ? .replace("%", "") }
        color = "#00c896" / >
        <
        ConfBar label = "Strabismus"
        value = { probStrab ? .replace("%", "") }
        color = "#ff4757" / >
        <
        /div>

        <
        p className = "result-disclaimer" > ⚠️Screening tool only— not a medical diagnosis. <
        /p> <
        /div>
    );
}
...
function WebcamModal({ onCapture, onClose }) {
    ...
    return ( <
        ...
        /div> <
        video ref = { videoRef }
        autoPlay playsInline className = "webcam-video" / >
        <
    ...
    function AboutSection() {
        ...
        return ( <
            ...
            h2 className = "about-heading" > What is < br / > < em > Strabismus ? < /em></h
            2 >
            <
            ...
            or permanent vision loss.Early detection— especially in children— significantly improves treatment outcomes. <
            /p> <
            div className = "about-note" >
            <
            ScanIcon / >
            <
            span > This AI tool is a screening aid, not a substitute
            for clinical examination. < /span> <
            /div> <
            /div> <
    ...
    export default function App() {
        ...
        return ( <
            ...
            Upload an eye photo or use your webcam.Our CNN model analyses eye alignment and returns an instant screening result. <
            /p> <
            /div>
            ...
            { /* Left: drop zone */ } <
            div className = { `drop-zone ${dragOver ? "drag-active" : ""} ${imageDataUrl ? "has-image" : ""}` }
            ...
            onDrop = { handleDrop }
            onClick = {
                () => !imageDataUrl && fileInputRef.current ? .click() } >
            {
                imageDataUrl ? ( <
                    >
                    <
...

            {
                ...
                {
                    showWebcam && ( <
                        WebcamModal onCapture = { handleWebcamCapture }
                        onClose = {
                            () => setShowWebcam(false) }
                        />
                    )
